...
import { ArrowLeft, MessageCircle, Minus, Plus, ShieldCheck, Zap, Star, Leaf, ShoppingBag, ChevronDown } from 'lucide-react';
...
const SpiceDetail = () => {
  ...
  // Assuming the 'price' in PURE_SPICES is for 100g (as per our previous update)
  const currentPrice = useMemo(() => {
    if (!spice) return 0;
    const basePrice = spice.price; // Price for 100g
    if (selectedWeight === 50) return Math.round(basePrice * 0.55); // 50g slightly higher margin
    if (selectedWeight === 250) return Math.round(basePrice * 2.5); // 250g
    return basePrice;
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F4] text-[#1a1a1a] selection:bg-orange-100">
      ...
      <nav className="fixed top-0 left-0 w-full z-[100] p-4 md:p-10 pointer-events-none">
        <motion.button 
          onClick={() => router.back()}
          className="pointer-events-auto flex items-center gap-3 px-6 py-3 bg-white/80 backdrop-blur-2xl border border-black/5 rounded-full shadow-lg hover:bg-white transition-all group cursor-pointer"
        >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 md:px-12 pt-32 md:pt-40 pb-32">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start">
          
          {/* Left: Image Section */}
          <div className="relative lg:sticky lg:top-32 w-full">
            <motion.div className="relative aspect-[4/5] rounded-[50px] overflow-hidden shadow-2xl group">
          ...
          <div className="flex flex-col space-y-10">
            <header className="space-y-4">
              <div className="flex items-center gap-3">
                <span className="h-[1px] w-8 bg-orange-900/30" />
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-900/60">Selected Pure Spice</span>
              </div>
              <h1 className="text-6xl md:text-8xl font-serif leading-[0.9] tracking-tighter text-[#111111]">
              ...
              <div className="flex items-center gap-8 pt-4">
                <div className="flex flex-col">
                  ...
                  <p className="text-4xl font-medium text-[#111111]">Rs. {currentPrice * quantity}</p>
                </div>
                <div className="h-10 w-[1px] bg-black/10" />
                
                {/* --- PREMIUM WEIGHT DROPDOWN --- */}
                  ...
                  <AnimatePresence>
                    {isDropdownOpen && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute top-full left-0 mt-4 bg-white border border-black/5 shadow-2xl rounded-2xl p-2 z-50 min-w-[140px] backdrop-blur-xl"
                      >
                        {[50, 100, 250].map((w) => (
                          <button
                            key={w}
            ...
            <p className="text-base md:text-xl text-gray-500 font-light leading-relaxed italic border-l-2 border-orange-900/10 pl-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center border border-black/5 rounded-full p-2 bg-white shadow-sm w-fit">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-12 rounded-full flex items-center justify-center hover:bg-orange-50 cursor-pointer"><Minus size={16}/></button>
                <span className="w-16 text-center font-bold text-3xl tabular-nums">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="w-12 h-12 rounded-full flex items-center justify-center hover:bg-orange-50 cursor-pointer"><Plus size={16}/></button>
              </div>
            </div>

            {/* Desktop Buttons */}
            <div className="hidden lg:grid grid-cols-2 gap-4">
              <motion.button onClick={handleAddToCart} whileHover={{ y: -3 }} className="flex items-center justify-center gap-3 py-5 bg-white border border-black/10 rounded-full shadow-sm hover:bg-black hover:text-white transition-all cursor-pointer">
                <ShoppingBag size={18} />
                <span className="text-[10px] font-black uppercase tracking-widest">Add to Basket</span>
              </motion.button>
              <motion.a href={whatsappUrl} target="_blank" whileHover={{ y: -3 }} className="flex items-center justify-center gap-3 py-5 bg-[#111111] text-white rounded-full shadow-xl hover:bg-[#25D366] transition-all cursor-pointer">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full p-4 z-[110] flex gap-2 bg-gradient-to-t from-[#FBF9F4] to-transparent">
        <button onClick={handleAddToCart} className="flex-none p-5 bg-white border border-black/10 rounded-[20px] shadow-xl"><ShoppingBag size={22} /></button>
        <a href={whatsappUrl} target="_blank" className="flex-grow flex items-center justify-between px-6 py-5 bg-[#111111] text-white rounded-[20px] shadow-2xl">
