...
import { ArrowLeft, MessageCircle, Minus, Plus, ShieldCheck, Zap, Star, ShoppingBag, ChevronDown, CreditCard } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#FBF9F4] text-[#1a1a1a] selection:bg-orange-100">
      ...
      <main className="max-w-7xl mx-auto px-6 pt-44 md:pt-48 pb-32">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-start">
          ...
          <div className="relative lg:sticky lg:top-36 w-full">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative aspect-[4/5] rounded-[30px] md:rounded-[50px] overflow-hidden shadow-2xl group">
          ...
          <div className="flex flex-col space-y-10">
            <header className="space-y-4">
              ...
              <div className="flex items-center gap-10 pt-4">
                <div className="flex flex-col">
                  ...
                  <p className="text-4xl md:text-5xl font-medium tracking-tighter tabular-nums text-[#111111]">Rs. {totalPrice}</p>
                </div>
                ...
                <div className="relative">
                  ...
                  <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-3 text-xl font-serif italic text-orange-900/60 group cursor-pointer">
                    {selectedWeight}g Pack
                    <ChevronDown size={16} className={`transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {isDropdownOpen && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-0 mt-4 bg-white border border-black/5 shadow-2xl rounded-2xl p-2 z-[150] min-w-[140px] backdrop-blur-xl">
                        {[50, 100, 250].map((w) => (
                          <button key={w} onClick={() => { setSelectedWeight(w); setIsDropdownOpen(false); }} className={`w-full text-left px-4 py-3 rounded-xl text-[12px] font-bold uppercase tracking-widest transition-colors ${selectedWeight === w ? 'bg-orange-900 text-white' : 'hover:bg-orange-50'}`}>
                            {w}g
                          </button>
                        ))}
                      </motion.div>
            ...
            <div className="hidden lg:flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <motion.a href={whatsappUrl} target="_blank" whileHover={{ y: -5 }} className="flex items-center justify-center gap-3 py-6 bg-white border border-black/10 text-emerald-600 rounded-full hover:bg-emerald-600 hover:text-white transition-all duration-500 cursor-pointer">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full p-4 z-[110] bg-gradient-to-t from-[#FBF9F4] via-[#FBF9F4] to-transparent">
        <div className="max-w-md mx-auto flex items-center gap-2">
          {/* WhatsApp Circle */}
          <a href={whatsappUrl} target="_blank" className="flex-none p-5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-[20px] shadow-lg active:scale-95 transition-transform">
            <MessageCircle size={22} fill="currentColor" className="fill-emerald-100" />
          </a>

          {/* Cart Circle */}
          <button onClick={handleAddToCart} className="flex-none p-5 bg-white border border-black/10 rounded-[20px] shadow-lg active:scale-95 transition-transform">
            <ShoppingBag size={22} />
          </button>
