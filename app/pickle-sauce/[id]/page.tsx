    ...
    <div className="min-h-screen bg-[#FBF9F4] text-[#111111] selection:bg-orange-100">
      ...
      <main className="max-w-[1400px] mx-auto px-6 pt-44 md:pt-48 pb-32 lg:pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-16 lg:gap-24 items-start">
          ...
          <div className="lg:col-span-7 lg:sticky lg:top-36">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative aspect-[4/5] rounded-[30px] md:rounded-[50px] overflow-hidden shadow-2xl group bg-stone-100">
              <motion.img src={product.image} alt={product.name} className="w-full h-full object-cover scale-105 group-hover:scale-110 transition-transform duration-[3s] ease-out" />
              ...
              <header className="space-y-6">
                ...
                <div className="flex items-center gap-10 pt-2">
                  ...
                  <div className="relative">
                    ...
                    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 text-xl font-serif italic text-orange-900/60 leading-none group cursor-pointer">
                      ...
                      {isDropdownOpen && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-0 mt-4 bg-white border border-black/5 shadow-2xl rounded-2xl p-2 z-[120] min-w-[140px]">
                          {product.weights.map((w) => (
                            <button key={w.label} onClick={() => { setSelectedWeight(parseInt(w.label)); setIsDropdownOpen(false); }} className={`w-full text-left px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors ${selectedWeight === parseInt(w.label) ? 'bg-orange-900 text-white' : 'hover:bg-stone-50'}`}>
                              {w.label} gram
                            </button>
                          ))}
                        </motion.div>
              ...
              <div className="hidden lg:flex flex-col gap-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <motion.a href={whatsappUrl} target="_blank" whileHover={{ y: -5 }} className="flex items-center justify-center gap-3 py-6 bg-white border border-black/10 text-emerald-600 rounded-[25px] hover:bg-emerald-600 hover:text-white transition-all duration-500">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-widest">Direct Order</span>
                  </motion.a>
                </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full p-4 z-[110] bg-gradient-to-t from-[#FBF9F4] via-[#FBF9F4] to-transparent">
        <div className="max-w-md mx-auto flex items-center gap-2">
          {/* WhatsApp Circle */}
          <a href={whatsappUrl} target="_blank" className="flex-none p-5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-[22px] shadow-lg active:scale-95 transition-transform">
            <MessageCircle size={22} fill="currentColor" className="fill-emerald-100" />
          </a>

          {/* Cart Circle */}
          <button onClick={handleAddToCart} className="flex-none p-5 bg-white border border-black/10 rounded-[22px] shadow-lg active:scale-95 transition-transform">
            <ShoppingBag size={22} />
          </button>
          ...
          >
            ...
            <div className="flex items-center ">
              <span className="text-[9px] font-black uppercase tracking-widest item-center">Buy Now</span>
              <CreditCard size={18} />
            </div>
          </button>
        </div>
